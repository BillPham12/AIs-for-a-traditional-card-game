...
public class Rational_Patient implements RobotStrategy {
    ...
    public String[] play(Client c) {
        ...
        if (c.getCurrentMeld().getValue() == null){
            ...
            if (winning_meld != null && potential && winning_meld.size() <= 3) {
                if(winning_meld.size() == 1){
                    ...
                    if ((isBestPair(knowledge,winning_meld.get(1)) || isBestSequences(knowledge,winning_meld.get(1)))
                            || isBestCard(knowledge,winning_meld.get(1)))
                        play_meld = winning_meld.remove(1);
                    else if ( isBestCard(knowledge,winning_meld.get(0)) || (isBestPair(knowledge,winning_meld.get(0)) || isBestSequences(knowledge,winning_meld.get(0))))
        ...
        // The competitive turn
            ...
            //  System.out.println(HaveToPlay(c,current_meld));
            ...
            //System.out.println(potential_pairs + "---" + potential_sequences + "----"+ strategy);
         //   System.out.println( c.getCurrentMeld());
            List<String> king = new ArrayList<>(); king.addAll(potential_pairs); king.addAll(potential_sequences);
            ...
            if (card_left < 6 || possible == true) aggressive = true;
   ...
   /*     System.out.println("strategy" + strategy + " sequences" + sequences + " --- pairs " + pairs);*/
        ...
        if (sequences.size()  != 0 || pairs.size() != 0){
            if(card_left == 1){
               ...
               /* if (sequences.size() > 0 && support_functions.isVeryLowMeld(sequences.get(0))) toRemove =  2;
                else if (sequences.size() > 0 && support_functions.isVeryLowMeld(sequences.get(0))) toRemove = 2;
                else if (remove_cards.size() ==  2 &&
                        (support_functions.isLowMeld(remove_cards.get(0)) && support_functions.isLowMeld(remove_cards.get(1))))  toRemove = 1;
                else if (remove_cards.size() > 2 && support_functions.isLowMeld(remove_cards.get(0)))  toRemove = 1;
                else  if (sequences.size() >= 2 && sequences.get(0).length() == sequences.get(1).length()) toRemove = 2;
                else if (pairs.size() > sequences.size() + 1) toRemove = 3;*/
    ...
    private boolean HaveToPlay(Client c,String[] current_meld){
        ...
        boolean decide = true;
        if(high_rank && support_functions.findAGun(c.getHand()).size() ==0)
            decide = false;
        ...
        if(num >= x && support_functions.findAGun(c.getHand()).size() ==0 ){
            return true;}
        if(c.getHand().size() < 8 &&support_functions.findAGun(c.getHand()).size() ==0)
            decide = true;
        return decide;
    }
